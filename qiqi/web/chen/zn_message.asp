...
if keyword <>"" then
sql="select * from "&jieducm&"_china_message where ( messagename like '%" & keyword & "%' or  messagelxfs like '%" & keyword & "%' or messagetext like '%" & keyword & "%' ) and zn='yes'order by id desc"
else
sql="select * from "&jieducm&"_china_message where zn='yes' order by id desc"
end if
  ...
  <tr>
    <td height="20" bgcolor="#799AE1" align="center">
      <table width="98%"  border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td align="center"><font color="#FFFFFF" style="font-size:14px">站 内 消 息 管 理</font></td>
...
response.write"<tr bgcolor=#FFFFFF><td colspan='10'><p align='center'><font color='red'>暂无站内消息！</font></td></tr></table><br>"
response.end
else
const maxperpage=20
dim currentpage
...
do while i< maxperpage and not rs.eof%>
          <tr bgcolor="#FFFFFF">
            ...
            <td valign="top"><%=rs("messagename")%> </td>
            <td valign="top"><%if rs("uid")="all" then%>全部会员<%else%><%=rs("uid")%><%end if%></td>
            <td valign="top"><%=rs("messagelxfs")%></td>
            <td valign="top"><%=rs("messagetext")%></td>
            <td><p align="center"><%=rs("messagedate")%></td>
