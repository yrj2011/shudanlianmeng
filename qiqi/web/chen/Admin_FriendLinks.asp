...
if ID<>"" then
	if Action="Check" then
		conn.execute "Update "&jieducm&"_FriendLinks set IsOK=True where ID=" & CLng(ID)
	elseif Action="CancelCheck" then
		conn.execute "Update "&jieducm&"_FriendLinks set IsOK=False Where ID=" & CLng(ID)
	elseif Action="Good" then
		conn.execute "Update "&jieducm&"_FriendLinks set IsGood=True Where ID=" & CLng(ID)
	elseif Action="CancelGood" then
		conn.execute "Update "&jieducm&"_FriendLinks set IsGood=False Where ID=" & CLng(ID)
	elseif Action="Del" then
		conn.execute "Delete From "&jieducm&"_FriendLinks Where ID=" & CLng(ID)
	end if
end if
%>
...
sub main()
	sql="select * from "&jieducm&"_FriendLinks "
	if LinkType<>"" then
  ...
  <table width="90%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#000000" class="border">
    ...
    <tr bgcolor="#FFFFFF" class="tdbg"> 
      ...
      <td height="25"> 
        ...
        <font color="#FF0000"> *</font>(如：七七传媒)</td>
    </tr>
    <tr bgcolor="#FFFFFF" class="tdbg"> 
      ...
      <td height="25"> 
        ...
        <font color="#FF0000">*</font>(如：http://www.778892.com)</td>
    </tr>
    <tr bgcolor="#FFFFFF" class="tdbg"> 
...
sub Modify()
	...
	dim sqlLink,rsLink
	sqlLink="select * from "&jieducm&"_FriendLinks where ID=" & ID
...
<%
sub SaveAdd()
	dim rslink,LinkType,LinkSiteName,LinkSiteUrl,LinkLogoUrl,LinkSiteAdmin,LinkEmail,LinkSitePassword,LinkSitePwdConfirm,LinkSiteIntro,LinkIsGood,LinkIsOK
	LinkType=trim(request("LinkType"))
	...
	LinkIsOK=trim(request("IsOK"))
			...
			rsLink("LogoUrl")=LinkLogoUrl
			rsLink("SiteAdmin")=LinkSiteAdmin
			rsLink("Email")=LinkEmail			
			rsLink("SiteIntro")=LinkSiteIntro
