...
function ConfirmDel()
{
	...
	{
		document.myform.action="admin_recorddel.asp";
...
* <A href="admin_Record.asp">所有操作记录列表</A> 
* <A href="admin_Record.asp?action=ren">任务操作列表</A>  
* <A href="admin_Record.asp?action=chong">充值操作列表</A>
* <A href="admin_Record.asp?action=zeng">增值操作列表</A>
* <A href="admin_Record.asp?action=ti">提现操作列表</A>
* <A href="admin_Record.asp?action=login">登录列表</A>
* <A href="admin_Record.asp?action=manage">管理操作列表</A>
</DIV>
</DIV>
<table border=0 width=100% align=center bgcolor="#FFFFFF">
  <tr>
            <td width="100%"> 
              <div align="center">
                <form name="form1" method="post" action="admin_record.asp?action=ok">
                  
                  <IMG src="../../images/search.gif" width="16" height="16" align=absMiddle>搜索流水号：
...
if action="" then
sql="select * from "&jieducm&"_record  order by id desc"
elseif action="ren" then
sql="select * from "&jieducm&"_record where class='淘宝区任务' or class='拍拍区任务' or class='有啊区任务' order by id desc"
elseif action="chong" then
sql="select * from "&jieducm&"_record where  class='充值卡充值' or class='网银充值' or  class='支付宝充值'  or class='易宝充值' order by id desc"
elseif action="zeng" then
sql="select * from "&jieducm&"_record where  class='发布点赠送' or class='购买发布点' or class='积分换发布点'  or class='发布点换存款'  order by id desc"
elseif action="ti" then
sql="select * from "&jieducm&"_record where class='申请提现'  order by id desc"
elseif action="login" then
sql="select * from "&jieducm&"_record where class='登录信息'  order by id desc"
...
if not rs.eof then
	...
	if action="" then
	   url="admin_record.asp"
	elseif action="ren" then
	   url="admin_record.asp?action=ren"
    elseif action="chong" then
	  	 url="admin_record.asp?action=chong"
    elseif action="zeng" then
	 	 url="admin_record.asp?action=zeng"
    elseif action="ti" then
		 url="admin_record.asp?action=ti"
   elseif action="manage" then
    	 url="admin_record.asp?action=manage"
   elseif action="login" then
       	 url="admin_record.asp?action=login"
   elseif action="ok" then
   	 url="admin_record.asp?action="&request("text")&""
  end if
	 rs.pagesize=20
...
<UL class=missionitem>
  ...
  <LI style="WIDTH: 20%"><input name='ID' type='checkbox' onClick="unselectall()" id="ID" style="border: 0px;background-color: #eeeeee;" value=<%=rs("id")%>><%=rs("num")%> </LI>
