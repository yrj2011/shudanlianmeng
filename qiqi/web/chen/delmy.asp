...
conn.close
set conn=nothing		
response.Redirect("admin_mymission.asp")
      
elseif action="del" then

      Sqli = "select * from "&jieducm&"_zhongxin where id="&id&" and start='0'"
	  Set rs = Server.CreateObject("Adodb.RecordSet")
...
sqlr="update "&jieducm&"_register set fabudian=fabudian+"&fabu&" ,cunkuan=cunkuan+"&price&" where username='"&username&"'"
conn.execute sqlr
	  ...
	  rs.open "Select * From "&jieducm&"_record " ,Conn,3,3  
    	...
    	rs("num")=num
		rs("class")="清理删除"
		rs("content")=session("AdminName")&"进行清理删除"&id&"任务,返还存款"&price&"元,发布点："&fabu&"个"
		rs("price")=price
		rs("jiegou")="清理删除成功"
    	rs.update
    	rs.close
...
elseif action="back" then
      ...
      Set rs=server.createobject("ADODB.RECORDSET")
      rs.open "select start from "&jieducm&"_zhongxin where pid='"&id&"'",conn,3,3
       if not (rs.eof) then
		   ...
		   Set rs=server.createobject("ADODB.RECORDSET")
           rs.open "select start from "&jieducm&"_jieshou where pid='"&id&"'",conn,3,3
           if not (rs.eof) then
		   rs("start")=int(rs("start"))-1
           rs.update
		   rs.close
