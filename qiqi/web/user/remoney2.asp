...
elseif ReRName="" then
	 ...
	 response.End()
elseif ReZfb="" then
     Response.Write("<script>alert('的财付通账号不能为空!');history.back();</script>")
	 response.End()
elseif int(ReNum)<10 then
	Response.Write("<script>alert('低于10元的提现，请以发布任务的形式，让别人接手你的任务，存款就转到你的账户了!');history.back();</script>")
    response.End()
...
call check_jieducm_name(session("useridname"))				
	  ...
	  rs.open "Select * From "&jieducm&"_record " ,Conn,3,3  
    	...
    	rs.update
    	rs.close
    Response.Write("<script>alert('您已成功申请提现,\n我们将在第二个工作日将提现转进您的财付通上!');window.location.href='ReMoneyList.asp';</script>")	
	response.End()
set rs=nothing
end if
...
function  save_onclick22()
{
 ...
 if(!Number(Price))
	  
  {
	...
	if(Price<10)
	{
	alert("低于10元的提现，请以发布任务的形式，让别人接手你的任务，存款就转到您的财付通了!");
	document.formt.ReNum1.focus();
	return false;
	}
    ...
    <tr>
      <td height="5"></td>
                    ...
                    <div style="MARGIN-TOP: 10px; LINE-HEIGHT: 250%">
                      <ul>
                        <li>* 平台提供三种种提现方式，淘宝商品提现和支付宝提现和财付通。<br>
                          * 您也可以在提现列表内撤销提现，将自动为您立即返回金额至您用户名。<br>
                          *   您的操作记录中也会有相应的记录信息。<br>
                          *   由于提现量大，商品链接低于10元、财付通低于10元的提现，请以发布任务的形式，让别人接手您的任务，存款就转到您的店铺财付通了！ </li>
                      </ul>
                    </div><br><br>
...
<form name="formt" action="" method="post" onSubmit="return save_onclick22();">
                ...
                <tr>
                  <td height="35" colspan="2" align="left"><div align="center">每次只能进行一次提现，本次提现完成后才能进行下一次提现！</div></td>
                  </tr>
                <tr>
				  ...
				  淘宝商品地址提现                  
                  <input name="radiobutton" type="radio" onClick="javascript:location.href='remoney1.asp';" value="2" />
                  支付宝提现                <input type="radio" name="class" value="3" checked  />
                  财付通提现       </td>
                </tr>
				<tr>
                ...
                <tr>
                  <td align="right">&nbsp;</td>
                  <td align="left" height=30><A href="https://www.tenpay.com/v2/account/receive/index.shtml?pay_uin=xieshanhui@163.com&amount=&remark=提现: <%=session("useridname")%>" target=_blank> <img src="/user/images/tx.jpg" ></A></td>
                </tr>
				<tr>
                  <td height="35" align="right">&nbsp;</td>
                </tr>
                ...
                <tr>
                  <td height="35"  align="right">你的财付通账号：</td>
