...
var idxiao=id;
sAlert("<div style=margin:10px;line-height:140px;><FORM id=xiaohao method=post  name=formf><div style=line-height:20px;>  【已绑定的淘宝号】<br><%	
...
function sAlert(txt){
    ...
    strHtml += "    <li style=\"background:#DD828D;text-align:left;padding-left:20px;font-size:14px;font-weight: bold;color:#fff;height:25px;line-height:25px;border:1px solid #F9CADE;\">[请选择你购买此商品的淘宝用户名：]</li>\n";
...
param ='&'+	param;	   
   ...
   $.post('/taobao/jieducm-ajax.asp?open=indexajex',function (data) {
$('#content').html(data);
   $('#waitingimage').hide();
...
<UL id=task_nav>
  <LI><A style="FONT-SIZE: 16px; BACKGROUND: url(../images/task_nav_02.gif) no-repeat 50% bottom; COLOR: #ffffff"  href="javascript:void(0)" onClick="reloadTask()">淘宝互动区</A> </LI>
...
<A href="?sort=7"><SPAN class=anniu>立即收货区</SPAN></A>
<A href="?sort=6"><SPAN class=anniu>延迟收货区</SPAN></A>
...
action=request("sort")
if action="" then
  Sql = "select top 100  username,price,pid,bei,baohu2,start,now,isprice,tips,addfabu,jieducm_fb,IfComeSet from "&jieducm&"_zhongxin where classid='1' and datediff(n,now,'"&now&"')>=0 order by  start asc,now desc"
elseif action=0 then
  Sql = "select top 100 username,price,pid,bei,baohu2,start,now,isprice,tips,addfabu,jieducm_fb,IfComeSet from "&jieducm&"_zhongxin where  classid='1' and start='0' and datediff(n,now,'"&now&"')>=0 order by now, start  asc"
elseif action=1 then 
  Sql = "select top 100 username,price,pid,bei,baohu2,start,now,isprice,tips,addfabu,jieducm_fb,IfComeSet from "&jieducm&"_zhongxin where  classid='1' and  start='0' and datediff(n,now,'"&now&"')>=0 order by Price desc"
elseif action=2 then
  Sql = "select top 100 username,price,pid,bei,baohu2,start,now,isprice,tips,addfabu,jieducm_fb,IfComeSet from "&jieducm&"_zhongxin where   classid='1' and start='0' and datediff(n,now,'"&now&"')>=0 order by Price  asc"
elseif action=3 then
  Sql = "select top 100 username,price,pid,bei,baohu2,start,now,isprice,tips,addfabu,jieducm_fb,IfComeSet from "&jieducm&"_zhongxin  where  classid='1' and datediff(n,now,'"&now&"')>=0 and  price >= 1  and price<=30 and start='0' order by pid desc"
elseif action=4 then
  Sql = "select top 100 username,price,pid,bei,baohu2,start,now,isprice,tips,addfabu,jieducm_fb,IfComeSet from "&jieducm&"_zhongxin where  classid='1' and datediff(n,now,'"&now&"')>=0 and  price >30  and price<=100 and start='0' order by pid desc"
elseif action=5 then
  Sql = "select top 100 username,price,pid,bei,baohu2,start,now,isprice,tips,addfabu,jieducm_fb,IfComeSet from "&jieducm&"_zhongxin where   classid='1' and price >100  and start='0' and datediff(n,now,'"&now&"')>=0"
ElseIf action=6 Then
  Sql = "select top 100  username,price,pid,bei,baohu2,start,now,isprice,tips,addfabu,jieducm_fb,IfComeSet from "&jieducm&"_zhongxin where classid='1' and datediff(n,now,'"&now&"')>=0 order by  start asc,now desc"
  ElseIf action=7 Then
  Sql = "select top 100  username,price,pid,bei,baohu2,start,now,isprice,tips,addfabu,jieducm_fb,IfComeSet from "&jieducm&"_zhongxin where classid='1' and start='0'  and bei='马上收货好评' and datediff(n,now,'"&now&"')>=0  order by id desc"
ElseIf action=8 Then
  Sql = "select top 100  username,price,pid,bei,baohu2,start,now,isprice,tips,addfabu,jieducm_fb,IfComeSetfrom "&jieducm&"_zhongxin where classid='1' and start='0' and bei<>'马上收货好评' and datediff(n,now,'"&now&"')>=0  order by id desc"
end if
Set Rs = Server.CreateObject("Adodb.RecordSet")
Rs.Open Sql,conn,1,1
if not rs.eof then				
...
<%call xinyu(jifei)%>
	...
	平台担保 <img src="../images/jieducm_zf.gif" width="13" height="16"><font color="#FF0000"><strong><%=Price%></strong></font>元<br><%=isprice%></TD>
    <TD align=middle width="15%">
	<%=bei%><BR>
<%
if bei="马上收货好评"  then
...
call jieducm_fadian(jieducm_fb,addfabu)
 %></strong></font>个</TD>
	...
	<%if start=0 and baohu2=1 then%>
	<img src="../images/jieducm_baohu.gif" border="0" alt="此任务开启了任务保护,需要审核"><br /><%end if%>
