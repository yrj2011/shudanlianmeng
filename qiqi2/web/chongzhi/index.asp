...
rs.open "select top 1 num from "&jieducm&"_jieshou where username='"&session("useridname")&"' order by  id desc",conn,1,1
if not (rs.eof) then
  checknum=rs("num")
rs.close
end if
...
var idxiao=id;
sAlert("<div style=margin:10px;line-height:40px;><FORM id=xiaohao method=post><div style=line-height:40px;>  【已绑定的淘宝号】<br><%	
			  	Sql = "select * from "&jieducm&"_xinyu  where username='"&session("useridname")&"' and class=1 and start=0 order by id desc "
				Set Rs = Server.CreateObject("Adodb.RecordSet")
				Rs.Open Sql,conn,1,1
				IF Rs.Eof Then
				    wei=0
					Response.Write("<a href=../taobao/buyhao.asp>对不起，你还没有绑定买号！绑定买号后，才可以接手任务!</a>")
				Else
				    wei=1
					Do While Not Rs.Eof
...
function sAlert(txt){
    ...
    this.doOk = function(){
        ...
        document.body.onselectstart = function(){return true;}
        document.body.oncontextmenu = function(){return true;}
		window.location.href='../taobao/buyhao.asp'
    }
...
param ='&'+	param;	   
   ...
   $.post('/chongzhi/jieducm-ajax.asp?open=indexajex',function (data) {
$('#content').html(data);
   $('#waitingimage').hide();
...
<UL id=task_nav>
  <LI><A style="FONT-SIZE: 16px; BACKGROUND: url(../images/task_nav_02.gif) no-repeat 50% bottom; COLOR: #ffffff" href="index.asp">充值提现区</A> </LI>
  <LI><A  href="pushmission.asp">我要提现</A> </LI>
...
<A href="?sort=1"><SPAN class=anniu>价高排列</SPAN></A>
 <A href="?sort=2"><SPAN class=anniu>价低排列</SPAN></A>
  <A href="?sort=3"><SPAN class=anniu>100元区</SPAN></A>
  <A href="?sort=4"><SPAN class=anniu>200元区</SPAN></A>
<A href="?sort=5"><SPAN class=anniu>200元以上区</SPAN></A></DIV>
...
elseif action=0 then
  Sql = "select top 100 username,price,pid,bei,start,now,isprice,tips from "&jieducm&"_zhongxin where  classid='5' and start<>'5'  order by now, start  asc"
elseif action=1 then 
  Sql = "select top 100 username,price,pid,bei,start,now,isprice,tips from "&jieducm&"_zhongxin where  classid='5' and  start='0' order by Price desc"
elseif action=2 then
  Sql = "select top 100 username,price,pid,bei,start,now,isprice,tips from "&jieducm&"_zhongxin where   classid='5' and start='0' order by Price  asc"
elseif action=3 then
  Sql = "select top 100 username,price,pid,bei,start,now,isprice,tips from "&jieducm&"_zhongxin  where  classid='5' and  price = 100  and start='0' order by pid desc"
elseif action=4 then
  Sql = "select top 100 username,price,pid,bei,start,now,isprice,tips from "&jieducm&"_zhongxin where  classid='5' and  price >200 and start='0' order by pid desc"
elseif action=5 then
  Sql = "select top 100 username,price,pid,bei,start,now,isprice,tips from "&jieducm&"_zhongxin where   classid='5' and price >200  and start='0' "
ElseIf action=6 Then
  Sql = "select top 100  username,price,pid,bei,start,now,isprice,tips from "&jieducm&"_zhongxin where classid='5'  order by start asc,now desc"
end if
Set Rs = Server.CreateObject("Adodb.RecordSet")
Rs.Open Sql,conn,1,1
if not rs.eof then				
