...
* <A href="admin_Recordu.asp?username=<%=username%>">所有操作记录列表</A> 
* <A href="admin_Recordu.asp?username=<%=username%>&action=ren">任务操作列表</A>  
* <A href="admin_Recordu.asp?username=<%=username%>&action=chong">充值操作列表</A>
* <A href="admin_Recordu.asp?username=<%=username%>&action=zeng">增值操作列表</A>
* <A href="admin_Recordu.asp?username=<%=username%>&action=ti">提现操作列表</A>
* <A href="admin_Recordu.asp?username=<%=username%>&action=login">登录列表</A>
* <A href="admin_Recordu.asp?username=<%=username%>&action=manage">管理操作列表</A>
</DIV>
</DIV>
<DIV 
...
sql="select * from "&jieducm&"_record where username='"&username&"' and (class='淘宝区任务' or class='VIP区任务') order by id desc"
elseif action="chong" then
sql="select * from "&jieducm&"_record where username='"&username&"' and (class='充值卡充值' or class='网银充值' or  class='支付宝充值'  or class='易宝充值')  order by id desc"
elseif action="zeng" then
sql="select * from "&jieducm&"_record where username='"&username&"' and (class='发布点赠送' or class='购买发布点'  or class='积分换发布点'  or class='发布点换存款')  order by id desc"
elseif action="login" then
sql="select * from "&jieducm&"_record where username='"&username&"' and class='登录信息'  order by id desc"
elseif action="ti" then
sql="select * from "&jieducm&"_record where username='"&username&"' and class='申请提现'  order by id desc"
elseif action="manage" then
sql="select * from "&jieducm&"_record where username='"&username&"' and (class='清理买家' or class='清理删除' or class='返回上级' or class='确认发货' or class='买方好评' or class='卖方完成') order by id desc"
