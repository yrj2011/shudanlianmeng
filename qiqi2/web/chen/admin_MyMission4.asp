...
<table border=0 width=100% align=center bgcolor="#FFFFFF">
  <tr>
            <td width="100%"> 
              <div align="center">
                <form name="form1" method="post" action="admin_mymission4.asp?sort=ok&classid=1">
...
style="FONT-WEIGHT: bolder; FONT-SIZE: 18px; COLOR: red">已发充值提现区任务</SPAN>&nbsp; 
* <A href="admin_MyMission4.asp">所有任务</A>
* <A href="admin_MyMission4.asp?sort=0">进行中任务</A>
*<A href="admin_MyMission4.asp?sort=1">等待接手</A>
*<A href="admin_MyMission4.asp?sort=2">等待支付</A>
*<A href="admin_MyMission4.asp?sort=3">等待发货</A>
*<A href="admin_MyMission4.asp?sort=4">等待接手好评</A>
*<A href="admin_MyMission4.asp?sort=5">等待发方确认</A>
*<A href="admin_MyMission4.asp?sort=6">已经完成</A>
</DIV>
...
if action="" then
			  	Sql = "select * from "&jieducm&"_zhongxin where  classid='5'  order by start  asc"
elseif action="0" then 
			  	Sql = "select * from "&jieducm&"_zhongxin where  start<>'0' and start<>'5' and  classid='5' order by start  asc"
elseif action="1" then 
			  	Sql = "select * from "&jieducm&"_zhongxin where start='0'  and  classid='5' order by start  asc"
elseif action="2" then
			  	Sql = "select * from "&jieducm&"_zhongxin where start='1'   and  classid='5' order by start  asc"
elseif action="3" then
			  	Sql = "select * from "&jieducm&"_zhongxin where  start='2'  and  classid='5' order by start  asc"
elseif action="4" then
			  	Sql = "select * from "&jieducm&"_zhongxin where  start='3'   and  classid='5' order by start  asc"
elseif action="5" then
			  	Sql = "select * from "&jieducm&"_zhongxin where  start='4'  and  classid='5'  order by start  asc"
elseif action="6" then
			  	Sql = "select * from "&jieducm&"_zhongxin where  start='5'  and  classid='5'  order by start  asc"
elseif action="ok" then
 sql="select * from "&jieducm&"_zhongxin where  classid='5' and (pid like '%"&request("text")&"%' or  username like '%"&request("text")&"%')   order by start asc"				
end if
...
if not rs.eof then
	...
	if action="" then
	 url="admin_mymission4.asp"
	elseif action="ok" then
	url="admin_mymission4.asp?sort=ok&text="&request("text")&""
	else
	 url="admin_mymission4.asp?sort="&action&""
	end if
	 rs.pagesize=20
...
<%end if%></LI>
   <LI style="WIDTH: 15%"><%if start="1" then%><a href="delmy4.asp?action=qing&id=<%=pid%>" onClick="return confirm('确定要操作吗？');">清理买家</a><%else%><font color="#999999">清理买家</font><%end if%>
|<%if start="0" then%><a href="delmy4.asp?action=del&id=<%=pid%>" onClick="return confirm('确定要删除操作吗？');">清理删除</a><%else%><font color="#999999">清理删除</font><%end if%>
<br>
<%if start="3" then%><a href="delmy4.asp?action=hao&id=<%=pid%>">买方好评</a><%else%><font color="#999999">买方好评</font><%end if%>
|<%if start="4" then%><a href="delmy4.asp?action=haook&id=<%=pid%>">卖方完成</a><%else%><font color="#999999">卖方完成</font><%end if%>
<br>
<%if start<>"5" and start<>"0"  and start<>"1" then%><a href="delmy4.asp?action=back&id=<%=pid%>">返回上级</a><%else%><font color="#999999">返回上级</font><%end if%>
|<%if start="2" then%><a href="delmy4.asp?action=ok&id=<%=pid%>">确认发货</a><%else%><font color="#999999">确认发货</font><%end if%></LI>
  </UL></DIV></td>
  </tr>
</table>
   <%
