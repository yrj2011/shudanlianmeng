...
elseif(int(fabudian2)<int(fabu2)) then
 ...
 response.End()
    	...
    	rs.close
		 ...
		 Response.Write("<script>alert('出错了!请重新发布');history.back();</script>")
         response.End()
		end if
...
conn.execute sql
     ...
     Set rs=server.createobject("ADODB.RECORDSET")
	  rs.open "Select * From "&jieducm&"_record " ,Conn,3,3  
    	...
    	rs("num")=pid
		rs("class")="拍拍区任务"
		rs("content")=session("useridname")&"从任务仓库中发布任务"&pid&"成功,发布点减少了"&jieducm_fb&"个"
		rs("price")="-"&price
		rs("jiegou")="成功"
    	rs.update
    	rs.close
		call check_jieducm_name(session("useridname")) 
	Response.Write("<script>alert('发布成功!');window.location.href='MyMission.asp';</script>")
	response.End()
end if
%>
...
<UL id=paipai_task_nav>
  ...
  <LI><A href="javascript:getPart('ReMission1.asp',<%=request("pageno")%>,<%=request("sort")%>);">已接任务</A> </LI>
  <LI><A href="MyMission.asp">已发任务</A> </LI>
  <LI><A href="Buyhao.asp">绑定买号</A> </LI>
   <LI><A href="MyWw.asp">绑定店铺</A> </LI>
  <LI><A style="FONT-SIZE: 16px; BACKGROUND: url(../images/paipai_task_nav_02.gif) no-repeat 50% bottom; COLOR: #ffffff"  href="MySave.asp">任务仓库</A> </LI></UL>
  </DIV>
</DIV>
