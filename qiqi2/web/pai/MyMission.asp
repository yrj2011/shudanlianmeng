...
param ='&'+	param;	   
   ...
   $.post('/pai/jieducm-ajax.asp?open=myajex',function (data) {
$('#content').html(data);
   $('#waitingimage').hide();
...
<UL id=paipai_task_nav>
  ...
  <LI><A style="FONT-SIZE: 16px; BACKGROUND: url(../images/paipai_task_nav_02.gif) no-repeat 50% bottom; COLOR: #ffffff" href="javascript:void(0)" onClick="reloadTask()">已发任务</A> </LI>
...
if action="" then
Sql = "select * from "&jieducm&"_zhongxin where classid='2'  and username='"&session("useridname")&"'  order by start  asc ,id desc"
elseif action="6" then
Sql = "select * from "&jieducm&"_zhongxin where classid='2'  and username='"&session("useridname")&"'  order by start  asc ,now desc"
else
Sql = "select * from "&jieducm&"_zhongxin where classid='2'  and username='"&session("useridname")&"' and start='"&action&"' order by start  asc ,id desc"
end if
...
nowf=rs("now")
 ...
 addfabu=0
 %> 			
<DIV class=missionbg style="WIDTH: 98%; PADDING-TOP: 10px; BORDER-BOTTOM: #06314a 1px dashed; HEIGHT: 140px">
...
[拉黑]</font></a><br />[<a href="../user/sms.asp?fausername=<%=username%>" target="_blank"><font color="#FF0000">发手机短信</font></a>]
<%
rs1.close
End IF
rs2.close
...
response.Write(nowj)
response.Write("<br><font color=#FF0000>接手拍拍号：</font>")
response.Write(numj)
%>
...
<div align="left">店铺动态打分：<%=play%>, 
 <%=bei%>!<font color="#FF0000"> 
 <%if j<>0 then%>本任务已免费提供<%=rs("company")%>单号:<%=rs("numbers")%><%end if%> </font></div>
</DIV>
<%if rs("tips")<>"" then%>
<DIV style="WIDTH: 98%; LINE-HEIGHT: 20px; PADDING-TOP: 2px;  TEXT-ALIGN: center">  
<div align="left">  
