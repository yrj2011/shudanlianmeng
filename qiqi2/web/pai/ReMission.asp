...
param ='&'+	param;	   
   ...
   $.post('/pai/jieducm-ajax.asp?open=Reajex',function (data) {
$('#content').html(data);
   $('#waitingimage').hide();
...
<UL id=paipai_task_nav>
  ...
  <LI><A style="FONT-SIZE: 16px; BACKGROUND: url(../images/paipai_task_nav_02.gif) no-repeat 50% bottom; COLOR: #ffffff" href="javascript:void(0)" onClick="reloadTask()">已接任务</A> </LI>
...
Sql = "select * from "&jieducm&"_jieshou where classid='2'  and username='"&session("useridname")&"'  order by start  asc"
else
Sql = "select * from "&jieducm&"_jieshou where classid='2'  and username='"&session("useridname")&"' and start='"&action&"' order by start  asc"
end if
